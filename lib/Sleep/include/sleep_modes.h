...
#define DEEP_SLEEP_WAKEUP_DELAY CONFIG_ESP32_DEEP_SLEEP_WAKEUP_DELAY
    ...
    defined(CONFIG_ESP32S3_DEEP_SLEEP_WAKEUP_DELAY)
...
extern void
periph_inform_out_light_sleep_overhead(uint32_t out_light_sleep_time);
...
#define CHECK_SOURCE(source, value, mask)                                      \
    ((s_config.wakeup_triggers & mask) && (source == value))
...
typedef struct {
    ...
    uint32_t
        ext1_rtc_gpio_mask : 22; // 22 is the maximum RTCIO number in all chips
...
_Static_assert(22 >= SOC_RTCIO_PIN_COUNT,
               "Chip has more RTCIOs than 22, should increase "
               "ext1_rtc_gpio_mask field size");
...
/* Updating RTC_MEMORY_CRC_REG register via set_rtc_memory_crc()
   is not thread-safe, so we need to disable interrupts before going to deep
   sleep. */
...
// in this mode, 2uA is saved, but RTC memory can't use at high temperature, and
// RTCIO can't be used as INPUT.
...
static void RTC_IRAM_ATTR __attribute__((used, noinline))
esp_wake_stub_start(void) {
...
static void __attribute__((section(".rtc.entry.text")))
esp_wake_stub_entry(void) {
#define _SYM2STR(s) #s
...
#else
    esp_deep_sleep_wake_stub_fn_t stub_ptr =
        (esp_deep_sleep_wake_stub_fn_t)REG_READ(RTC_ENTRY_ADDR_REG);
#endif
    if (!esp_ptr_executable(reinterpret_cast<void *>(stub_ptr))) {
        return NULL;
    }
    return stub_ptr;
}
...
void esp_wake_deep_sleep(void) { esp_default_wake_deep_sleep(); }
...
static void IRAM_ATTR flush_uarts(void) {
    for (int i = 0; i < SOC_UART_NUM; ++i) {
        ...
        if (periph_ll_periph_enabled(
                (periph_module_t)((int)PERIPH_UART0_MODULE + i))) {
            esp_rom_uart_tx_wait_idle(i);
        }
#endif
    }
...
static void IRAM_ATTR suspend_uarts(void) {
    for (int i = 0; i < SOC_UART_NUM; ++i) {
        ...
        } while (!(uart_fsm == UART_LL_FSM_IDLE ||
                   uart_fsm == UART_LL_FSM_TX_WAIT_SEND));
#else
...
inline static uint32_t IRAM_ATTR
call_rtc_sleep_start(uint32_t reject_triggers, uint32_t lslp_mem_inf_fpu);
...
static uint32_t IRAM_ATTR esp_sleep_start(uint32_t pd_flags) {
...
#endif
    // Keep the RTC8M_CLK on if the ledc low-speed channel is clocked by
    // RTC8M_CLK in lightsleep mode
    ...
    if (deep_sleep) {
        if (s_config.wakeup_triggers & RTC_TOUCH_TRIG_EN) {
            ...
            /* Workaround: In deep sleep, for ESP32S2, Power down the RTC_PERIPH
             * will change the slope configuration of Touch sensor sleep pad.
             * The configuration change will change the reading of the sleep
             * pad, which will cause the touch wake-up sensor to trigger
             * falsely.
             */
        ...
        /* In light sleep, the RTC_PERIPH power domain should be in the power-on
         * state (Power on the touch circuit in light sleep), otherwise the
         * touch sensor FSM will be cleared, causing touch sensor false
         * triggering.
         */
        if (touch_ll_get_fsm_state()) { // Check if the touch sensor is working
                                        // properly.
            pd_flags &= ~RTC_SLEEP_PD_RTC_PERIPH;
        }
    }
    ...
    if ((pd_flags & RTC_SLEEP_PD_DIG) == 0 &&
        ...
        /* Light sleep, enable sleep reject for faster return from this
         * function, in case the wakeup is already triggerred.
         */
#if CONFIG_IDF_TARGET_ESP32
        reject_triggers =
            RTC_CNTL_LIGHT_SLP_REJECT_EN_M | RTC_CNTL_GPIO_REJECT_EN_M;
#else
    ...
    uint32_t result;
    if (deep_sleep) {
#if !CONFIG_IDF_TARGET_ESP32H2 // IDF does not officially support esp32h2 in
                               // v4.4
        ...
        size_t rtc_fast_length =
            (size_t)_rtc_noinit_end - (size_t)_rtc_text_start;
#else
        extern char _rtc_force_fast_end[];
        size_t rtc_fast_length =
            (size_t)_rtc_force_fast_end - (size_t)_rtc_text_start;
#endif
        esp_rom_set_rtc_wake_addr((esp_rom_wake_func_t)esp_wake_stub_entry,
                                  rtc_fast_length);
        result = call_rtc_sleep_start(reject_triggers, config.lslp_mem_inf_fpu);
#else
#if !CONFIG_ESP_SYSTEM_ALLOW_RTC_FAST_MEM_AS_HEAP
        /* If not possible stack is in RTC FAST memory, use the ROM function to
         * calculate the CRC and save ~140 bytes IRAM */
        set_rtc_memory_crc();
        result = call_rtc_sleep_start(reject_triggers, config.lslp_mem_inf_fpu);
#else
        /* Otherwise, need to call the dedicated soc function for this */
        result =
            rtc_deep_sleep_start(s_config.wakeup_triggers, reject_triggers);
#endif
#endif // SOC_PM_SUPPORT_DEEPSLEEP_VERIFY_STUB_ONLY
    } else {
        result = call_rtc_sleep_start(reject_triggers, config.lslp_mem_inf_fpu);
...
#if CONFIG_IDF_TARGET_ESP32S2
    /* Due to hardware limitations, on S2 the brownout detector sometimes
       trigger during deep sleep to circumvent this we disable the brownout
       detector before sleeping  */
 ...
 */
static esp_err_t
esp_light_sleep_inner(uint32_t pd_flags, uint32_t flash_enable_time_us,
                      rtc_vddsdio_config_t vddsdio_config) IRAM_ATTR
 ...
 * 0                        |  0                                   |  no pd
 * flash x                        |  1                                   |  pd
 * flash with relaxed conditions(force_pd) 1                        |  0 |  pd
 * flash with strict  conditions(safe_pd)
 */
static inline bool
can_power_down_vddsdio(const uint32_t vddsdio_pd_sleep_duration) {
    bool force_pd = !(s_config.wakeup_triggers & RTC_TIMER_TRIG_EN) ||
                    (s_config.sleep_duration > vddsdio_pd_sleep_duration);
    bool safe_pd = (s_config.wakeup_triggers == RTC_TIMER_TRIG_EN) &&
                   (s_config.sleep_duration > vddsdio_pd_sleep_duration);
    return (s_config.pd_options[ESP_PD_DOMAIN_VDDSDIO] == ESP_PD_OPTION_OFF)
               ? force_pd
               : safe_pd;
}
...
esp_err_t esp_light_sleep_start(void) {
    ...
    /*
    Note: We are about to stall the other CPU via the
    esp_ipc_isr_stall_other_cpu(). However, there is a chance of deadlock if
    after stalling the other CPU, we attempt to take spinlocks already held by
    the other CPU that is.

    Thus any functions that we call after stalling the other CPU will need to
    have the locks taken first to avoid deadlock.
    ...
    */
    ...
    /* We will be calling esp_rtc_get_time_us() below. Make sure the code on the
     * other CPU is not holding the esp_rtc_get_time_us() lock, otherwise there
     * will be deadlock. esp_rtc_get_time_us() is called via:
     *
    ...
    uint32_t sleep_time_overhead_in =
        (ccount_at_sleep_start - s_config.ccount_ticks_record) /
    ...
    if (rtc_clk_slow_freq_get() == RTC_SLOW_FREQ_32K_XTAL) {
        ...
        s_config.rtc_clk_cal_period =
            (time_per_us << RTC_CLK_CAL_FRACT) / rtc_clk_slow_freq_get_hz();
    } else {
        // If the external 32 kHz XTAL does not exist, use the internal 150 kHz
        // RC oscillator as the RTC slow clock source.
        s_config.rtc_clk_cal_period =
            rtc_clk_cal(RTC_CAL_RTC_MUX, RTC_CLK_SRC_CAL_CYCLES);
        esp_clk_slowclk_cal_set(s_config.rtc_clk_cal_period);
    }
#elif defined(CONFIG_ESP32S2_RTC_CLK_SRC_INT_RC)
    s_config.rtc_clk_cal_period =
        rtc_clk_cal_cycling(RTC_CAL_RTC_MUX, RTC_CLK_SRC_CAL_CYCLES);
    esp_clk_slowclk_cal_set(s_config.rtc_clk_cal_period);
#else
    s_config.rtc_clk_cal_period =
        rtc_clk_cal(RTC_CAL_RTC_MUX, RTC_CLK_SRC_CAL_CYCLES);
    ...
    uint32_t rtc_cntl_xtl_buf_wait_slp_cycles = rtc_time_us_to_slowclk(
        RTC_CNTL_XTL_BUF_WAIT_SLP_US, s_config.rtc_clk_cal_period);
    s_config.sleep_time_adjustment =
        LIGHT_SLEEP_TIME_OVERHEAD_US + sleep_time_overhead_in +
        s_config.sleep_time_overhead_out +
        rtc_time_slowclk_to_us(rtc_cntl_xtl_buf_wait_slp_cycles +
    ...
    const uint32_t flash_enable_time_us =
        VDD_SDIO_POWERUP_TO_FLASH_READ_US + DEEP_SLEEP_WAKEUP_DELAY;

    /**
     * If VDD_SDIO power domain is requested to be turned off, bit
     * `RTC_SLEEP_PD_VDDSDIO` will be set in `pd_flags`.
     */
    if (pd_flags & RTC_SLEEP_PD_VDDSDIO) {
        /*
         * When VDD_SDIO power domain has to be turned off, the minimum sleep
         * time of the system needs to meet the sum below:
         ...
         *    min_protect(2);
         * 4. All the adjustment time which is s_config.sleep_time_adjustment
         * below.
         */
        const uint32_t vddsdio_pd_sleep_duration =
            MAX(FLASH_PD_MIN_SLEEP_TIME_US,
                flash_enable_time_us + LIGHT_SLEEP_MIN_TIME_US +
                    s_config.sleep_time_adjustment +
                    rtc_time_slowclk_to_us(RTC_MODULE_SLEEP_PREPARE_CYCLES,
                                           s_config.rtc_clk_cal_period));
    ...
    bool wdt_was_enabled =
        wdt_hal_is_enabled(&rtc_wdt_ctx); // If WDT was enabled in the user
                                          // code, then do not change it here.
    if (!wdt_was_enabled) {
        wdt_hal_init(&rtc_wdt_ctx, WDT_RWDT, 0, false);
        uint32_t stage_timeout_ticks =
            (uint32_t)(1000ULL * rtc_clk_slow_freq_get_hz() / 1000ULL);
        wdt_hal_write_protect_disable(&rtc_wdt_ctx);
        wdt_hal_config_stage(&rtc_wdt_ctx, WDT_STAGE0, stage_timeout_ticks,
                             WDT_STAGE_ACTION_RESET_RTC);
        wdt_hal_enable(&rtc_wdt_ctx);
        wdt_hal_write_protect_enable(&rtc_wdt_ctx);
    }

    int64_t final_sleep_duration_us = (int64_t)s_config.sleep_duration -
                                      (int64_t)s_config.sleep_time_adjustment;
    int64_t min_sleep_duration_us = rtc_time_slowclk_to_us(
        RTC_CNTL_MIN_SLP_VAL_MIN, s_config.rtc_clk_cal_period);

    // reset light sleep wakeup flag before a new light sleep
    s_light_sleep_wakeup = false;

    // if rtc timer wakeup source is enabled, need to compare final sleep
    // duration and min sleep duration to avoid late wakeup
    if ((s_config.wakeup_triggers & RTC_TIMER_TRIG_EN) &&
        (final_sleep_duration_us <= min_sleep_duration_us)) {
    ...
    // FRC1 has been clock gated for the duration of the sleep, correct for
    // that.
    ...
    uint64_t rtc_time_diff = rtc_time_slowclk_to_us(
        rtc_ticks_at_end - s_config.rtc_ticks_at_sleep_start,
        s_config.rtc_clk_cal_period);
    ...
    s_config.sleep_time_overhead_out =
        (cpu_ll_get_cycle_count() - s_config.ccount_ticks_record) /
        (esp_clk_cpu_freq() / 1000000ULL);
#endif // ENABLE_LIGHT_SLEEP
    return err;
}
...
esp_err_t esp_sleep_disable_wakeup_source(esp_sleep_source_t source) {
    ...
    } else if (CHECK_SOURCE(source, ESP_SLEEP_WAKEUP_TIMER,
                            RTC_TIMER_TRIG_EN)) {
    ...
    } else if (CHECK_SOURCE(source, ESP_SLEEP_WAKEUP_TOUCHPAD,
                            RTC_TOUCH_TRIG_EN)) {
        s_config.wakeup_triggers &= ~RTC_TOUCH_TRIG_EN;
#endif
    } else if (CHECK_SOURCE(source, ESP_SLEEP_WAKEUP_GPIO, RTC_GPIO_TRIG_EN)) {
        s_config.wakeup_triggers &= ~RTC_GPIO_TRIG_EN;
    } else if (CHECK_SOURCE(source, ESP_SLEEP_WAKEUP_UART,
                            (RTC_UART0_TRIG_EN | RTC_UART1_TRIG_EN))) {
        s_config.wakeup_triggers &= ~(RTC_UART0_TRIG_EN | RTC_UART1_TRIG_EN);
    }
#if defined(CONFIG_ESP32_ULP_COPROC_ENABLED) ||                                \
    defined(CONFIG_ESP32S2_ULP_COPROC_ENABLED) ||                              \
    defined(CONFIG_ESP32S3_ULP_COPROC_ENABLED)
...
#if CONFIG_IDF_TARGET_ESP32
#if ((defined CONFIG_ESP32_RTC_EXT_CRYST_ADDIT_CURRENT) ||                     \
     (defined CONFIG_ESP32_RTC_EXT_CRYST_ADDIT_CURRENT_V2))
    ESP_LOGE(TAG, "Failed to enable wakeup when provide current to external "
                  "32kHz crystal");
...
static void timer_wakeup_prepare(void) {
    ...
    int64_t ticks =
        rtc_time_us_to_slowclk(sleep_duration, s_config.rtc_clk_cal_period);
    rtc_hal_set_wakeup_timer(s_config.rtc_ticks_at_sleep_start + ticks);
}

#if CONFIG_IDF_TARGET_ESP32S2 || CONFIG_IDF_TARGET_ESP32S3
/* In deep sleep mode, only the sleep channel is supported, and other touch
 * channels should be turned off. */
static void touch_wakeup_prepare(void) {
    uint16_t sleep_cycle = 0;
    uint16_t meas_times = 0;
    touch_pad_t touch_num = TOUCH_PAD_NUM0;
    touch_ll_sleep_get_channel_num(
        &touch_num); // Check if the sleep pad is enabled.
    if ((touch_num > TOUCH_PAD_NUM0) && (touch_num < TOUCH_PAD_MAX) &&
        touch_ll_get_fsm_state()) {
        touch_ll_stop_fsm();
        touch_ll_clear_channel_mask(TOUCH_PAD_BIT_MASK_ALL);
        touch_ll_intr_clear(
            TOUCH_PAD_INTR_MASK_ALL); // Clear state from previous wakeup
...
touch_pad_t esp_sleep_get_touchpad_wakeup_status(void) {
    ...
    touch_pad_t pad_num;
    esp_err_t ret = touch_pad_get_wakeup_status(
        &pad_num); // TODO 723diff commit id:fda9ada1b
    assert(ret == ESP_OK &&
           "wakeup reason is RTC_TOUCH_TRIG_EN but SENS_TOUCH_MEAS_EN is zero");
...
static void ext1_wakeup_prepare(void) {
    ...
    rtc_hal_ext1_set_wakeup_pins(s_config.ext1_rtc_gpio_mask,
                                 s_config.ext1_trigger_mode);
}
...
static void esp_deep_sleep_wakeup_prepare(void) {
    for (gpio_num_t gpio_idx = GPIO_NUM_0; gpio_idx < GPIO_NUM_MAX;
         gpio_idx = (gpio_num_t)((int)gpio_idx + 1)) {
        if (((1ULL << gpio_idx) & s_config.gpio_wakeup_mask) == 0) {
            continue;
        }
#ifdef PULLUP_PULLDOWN_ON_DEEP_SLEEP
...
#endif // PULLUP_PULLDOWN_ON_DEEP_SLEEP
...
esp_err_t
esp_deep_sleep_enable_gpio_wakeup(uint64_t gpio_pin_mask,
    ...
    gpio_int_type_t intr_type =
        ((mode == ESP_GPIO_WAKEUP_GPIO_LOW) ? GPIO_INTR_LOW_LEVEL
                                            : GPIO_INTR_HIGH_LEVEL);
    esp_err_t err = ESP_OK;
    for (gpio_num_t gpio_idx = GPIO_NUM_0; gpio_idx < GPIO_NUM_MAX;
         gpio_idx = (gpio_num_t)((int)gpio_idx + 1), gpio_pin_mask >>= 1) {
        if ((gpio_pin_mask & 1) == 0) {
            continue;
        }
        if (!Sleep::esp_sleep_is_valid_wakeup_gpio((gpio_num_t)(gpio_idx))) {
            ESP_LOGE(
                TAG,
                "invalid mask, please ensure gpio number is no more than 5");
            return ESP_ERR_INVALID_ARG;
        }
...
#else
    uint32_t wakeup_cause =
        REG_GET_FIELD(RTC_CNTL_SLP_WAKEUP_CAUSE_REG, RTC_CNTL_WAKEUP_CAUSE);
#endif
...
static uint32_t get_power_down_flags(void) {
    ...
    // RTC_SLOW_MEM is needed for the ULP, so keep RTC_SLOW_MEM powered up if
    // ULP is used and RTC_SLOW_MEM is Auto. If there is any data placed into
    // .rtc.data or .rtc.bss segments, and RTC_SLOW_MEM is Auto, keep it powered
    // up as well.
     ...
     * So use a volatile variable to prevent compiler from doing this
     * optimization.
     */
    volatile size_t rtc_slow_mem_used = (size_t)&_rtc_slow_length;

    if ((s_config.pd_options[ESP_PD_DOMAIN_RTC_SLOW_MEM] ==
         ESP_PD_OPTION_AUTO) &&
        (rtc_slow_mem_used > 0 ||
         (s_config.wakeup_triggers & RTC_ULP_TRIG_EN))) {
    ...
    /* RTC_FAST_MEM is needed for deep sleep stub.
       If RTC_FAST_MEM is Auto, keep it powered on, so that deep sleep stub can
       run. In the new chip revision, deep sleep stub will be optional, and this
       can be changed. */
    ...
    // If RTC_PERIPH is left auto (EXT0/GPIO aren't enabled), RTC_PERIPH will be
    ...
    if (s_config.pd_options[ESP_PD_DOMAIN_VDDSDIO] != ESP_PD_OPTION_ON) {
     ...
     (defined CONFIG_ESP32_RTC_EXT_CRYST_ADDIT_CURRENT))
    if ((s_config.wakeup_triggers & (RTC_TOUCH_TRIG_EN | RTC_ULP_TRIG_EN)) ==
        0) {
        // If enabled EXT1 only and enable the additional current by touch,
        // should be keep RTC_PERIPH power on.
        pd_flags &= ~RTC_SLEEP_PD_RTC_PERIPH;
    }
#endif
...
void esp_deep_sleep_disable_rom_logging(void) { rtc_suppress_rom_log(); }

void esp_sleep_enable_adc_tsens_monitor(bool enable) {
    s_adc_tsen_enabled = enable;
}

void rtc_sleep_enable_ultra_low(bool enable) { s_ultra_low_enabled = enable; }
