...
DS18B20::DS18B20(uint8_t pin)
    : oneWire(OneWire(pin)), numberOfDevices(0), globalResolution(0),
      selectedResolution(0), selectedPowerMode(0) {
    resetSearch();
...
float DS18B20::getTempF() { return getTempC() * 1.8 + 32; }
...
uint8_t DS18B20::getPowerMode() { return selectedPowerMode; }

uint8_t DS18B20::getFamilyCode() { return selectedAddress[0]; }
...
uint8_t DS18B20::getNumberOfDevices() { return numberOfDevices; }
...
uint8_t DS18B20::hasAlarm() {
    ...
    return ((temp <= selectedScratchpad[ALARM_LOW]) ||
            (temp >= selectedScratchpad[ALARM_HIGH]));
}
...
int8_t DS18B20::getAlarmLow() { return selectedScratchpad[ALARM_LOW]; }
...
int8_t DS18B20::getAlarmHigh() { return selectedScratchpad[ALARM_HIGH]; }
...
int8_t DS18B20::getLowRegister() { return getAlarmLow(); }

void DS18B20::setLowRegister(int8_t lowRegister) { setAlarmLow(lowRegister); }

int8_t DS18B20::getHighRegister() { return getAlarmHigh(); }
...
uint8_t DS18B20::sendCommand(uint8_t romCommand, uint8_t functionCommand,
                             uint8_t power) {
